...
impl Var {
    ...
    pub fn min_f64(self, rhs: f64) -> Var {
        self.min(Var::new(rhs))
    }

    pub fn max_f64(self, rhs: f64) -> Var {
        self.max(Var::new(rhs))
    }

    pub fn f64_min(lhs: f64, rhs: Var) -> Var {
        Var::new(lhs).min(rhs)
    }

    pub fn f64_max(lhs: f64, rhs: Var) -> Var {
        Var::new(lhs).max(rhs)
    }
...
mod tests {
    ...
    fn min_max_test() {
        ...
        let y = Var::new(5.0);
        let m = x.min(y);
        let grad_m = backward(&m);
        assert!((grad_m[x.id()] - 1.0).abs() < 1e-12);
        assert!(grad_m[y.id()].abs() < 1e-12);
        reset_tape();
        let x = Var::new(3.0);
        let y = Var::new(5.0);
        let mx = x.max(y);
        let grad_mx = backward(&mx);
        assert!(grad_mx[x.id()].abs() < 1e-12);
        assert!((grad_mx[y.id()] - 1.0).abs() < 1e-12);
    }
}
